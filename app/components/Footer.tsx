...
export default function Footer() {
  ...
  const linkClasses = `
    text-sm
    text-amber-800/60 dark:text-[#9CA3AF]
    hover:text-amber-700 dark:hover:text-[#FFD166]
    transition-colors duration-200
  `;

  const headingClasses = `
    text-sm font-semibold uppercase tracking-wider
    text-amber-900 dark:text-[#F3F4F6]
  `;

  return (
    <footer className="relative border-t border-amber-200/60 dark:border-white/5 bg-[#FFFDF6]/70 dark:bg-[#080811]/90 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Brand Column */}
          <div className="space-y-4">
            <Link href="/" className="flex items-center gap-2 group">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-600 to-orange-600 dark:from-[#FFD166] dark:to-[#E0A96D] flex items-center justify-center shadow-sunlit-soft dark:shadow-glow-gold group-hover:scale-105 transition-transform">
                <Sparkles className="w-4 h-4 text-white dark:text-[#080811]" />
              </div>
              <span className="text-lg font-bold font-serif bg-gradient-to-r from-amber-600 to-orange-600 dark:from-[#FFD166] dark:to-[#E0A96D] bg-clip-text text-transparent">
                AstroVeda
              </span>
            </Link>
            <p className="text-sm text-amber-800/60 dark:text-[#9CA3AF] leading-relaxed">
