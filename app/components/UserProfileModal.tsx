...
export default function UserProfileModal({ isOpen, initialView, onClose }: UserProfileModalProps) {
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSaveMessage(error instanceof Error ? error.message : t('errors.api.generic'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleAvatarChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setUploadMessage(error instanceof Error ? error.message : t('errors.api.generic'));
    } finally {
    ...
    <form onSubmit={handleSave} className="space-y-5">
      <div className="flex items-center gap-4 border-b border-amber-200/60 pb-5 dark:border-white/10">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-amber-800/60 dark:text-[#9CA3AF]">{t('profile.accountEmail')}</p>
          {uploadMessage && (
          <p className={`mt-1 text-xs font-medium ${uploadMessage.includes("successfully") || uploadMessage.includes("सफलतापूर्वक") ? "text-emerald-700 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`} aria-live="polite">
            {uploadMessage}
          </p>
          )}
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="sm:col-span-2 text-sm font-medium text-amber-900/70 dark:text-[#D1D5DB]">
          ...
          <input className={`${inputClass} mt-1.5`} value={form.birthPlace} onChange={(event) => setForm({ ...form, birthPlace: event.target.value })} placeholder={t('profile.placePlaceholder')} />
        </label>
      </div>
    ...
    <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-lg border border-amber-200/70 bg-amber-50/70 p-4 dark:border-white/10 dark:bg-white/5">
            <p className="text-xs font-semibold uppercase text-amber-700/60 dark:text-[#9CA3AF]">{t('profile.walletBalance')}</p>
          ...
          <div className="rounded-lg border border-emerald-200 bg-emerald-50/70 p-4 dark:border-emerald-400/20 dark:bg-emerald-400/5">
            <p className="text-xs font-semibold uppercase text-emerald-700/70 dark:text-emerald-400">{t('profile.availableTokens')}</p>
            <p className="mt-2 text-2xl font-bold text-emerald-900 dark:text-emerald-300">{freeMessagesLeft}</p>
          </div>
        </div>
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="font-semibold text-amber-950 dark:text-[#F3F4F6]">{t('profile.transactions')}</h3>
            <p className="text-xs text-amber-800/60 dark:text-[#9CA3AF]">{t('profile.transactionsDesc')}</p>
          </div>
          <button onClick={() => { onClose(); openTopUp(); }} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-amber-600 px-3 py-2 text-sm font-semibold text-white dark:bg-[#FFD166] dark:text-[#080811]">
            <Plus className="h-4 w-4" /> {t('profile.addFunds')}
          </button>
      </div>
      {completedTopUps.length ? (
        ...
        <EmptyList icon={ReceiptText} title={t('profile.noTransactions')} description={t('profile.noTransactionsDesc')} />
      )}
    </div>
  );
      ...
      {kundaliHistory.map((entry) => (
        <div key={entry.id} className="rounded-lg border border-amber-200/70 p-4 dark:border-white/10">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="truncate font-semibold text-amber-950 dark:text-[#F3F4F6]">{t('profile.chartFor', { name: entry.name })}</h3>
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <HistoryStat label={t('kundali.ascendant')} value={entry.ascendant} />
            <HistoryStat label={t('kundali.moonSign')} value={entry.moonSign} />
            <HistoryStat label={t('kundali.nakshatra')} value={entry.nakshatra} />
            <HistoryStat label={t('kundali.placeOfBirth')} value={entry.placeOfBirth} />
          </dl>
        ...
        <div key={entry.id} className="rounded-lg border border-amber-200/70 p-4 dark:border-white/10">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 text-xs text-amber-800/60 dark:text-[#9CA3AF]">{t('profile.messagesCount', { count: entry.messages.length })} · {formatDate(entry.updatedAt)}</p>
            </div>
          </div>
        </div>
      ))}
    </div>
  ) : <EmptyList icon={MessageCircleMore} title={t('profile.noChatHistory')} description={t('profile.noChatHistoryDesc')} actionHref="/chat" actionLabel={t('profile.startChat')} />;
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="profile-modal-title" className="flex h-[92dvh] w-full flex-col overflow-hidden rounded-t-lg border border-amber-200/70 bg-[#FFFDF6] shadow-2xl sm:h-auto sm:max-h-[86vh] sm:max-w-5xl sm:rounded-lg dark:border-white/10 dark:bg-[#121026]">
        ...
        <div className="flex min-h-0 flex-1">
          <aside className="hidden w-56 shrink-0 border-r border-amber-200/60 p-3 sm:block dark:border-white/10">
            {sections.map(({ action, key, icon: Icon }) => (
              <button key={action} onClick={() => setActiveView(action)} className={`mb-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-semibold ${activeView === action ? "bg-amber-100 text-amber-950 dark:bg-[#FFD166]/10 dark:text-[#FFD166]" : "text-amber-900/60 hover:bg-amber-50 dark:text-[#9CA3AF] dark:hover:bg-white/5"}`}><Icon className="h-4 w-4" />{t(key)}</button>
            ))}
          </aside>
